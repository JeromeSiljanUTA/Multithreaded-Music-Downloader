...
musicbrainzngs.set_useragent(
    "Jerome's Music Scraper", "0.1", "jerome.siljan@mavs.uta.edu"
)
...
raw_releases = musicbrainzngs.get_artist_by_id(
    artist_id, includes=["release-groups"], release_type=["album", "ep"]
)
...
def add_tracks(title_num, discography, input_artist, album_list):
    ...
    album_id = (
        musicbrainzngs.search_releases(
            artist=input_artist, release=album_list[title_num], limit=1
        )
    )["release-list"][0]["id"]
    raw_tracks_list = musicbrainzngs.get_release_by_id(
        album_id, includes=["recordings"]
    )
...
for album in discography:
    if str(album_index_num) in arr_desired_albums:
        ...
        for song in album:
            executor.submit(
                ytdl, song_name=song, album_name=album_list[album_index_num]
            )
